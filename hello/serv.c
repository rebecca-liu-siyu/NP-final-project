...
int Game6(int roomID, Room_t* room) {
    ...
    // day comes
        ...
        printf("討論時間開始\n");
        broadcastMSG(room, "討論時間開始\n");

        for(int i = 0; i < room->capacity; i++){
            if(!alive[i] || !room->onlineStatus[i])
                continue;
            
            bzero(&sendline, MAXLINE);
            sprintf(sendline, "It's your turn to speak\n");
            writen(room->Connfds[i], sendline, strlen((sendline)));

            bzero(&recvline, MAXLINE);
            if(read(room->Connfds[i], recvline, MAXLINE) > 0){
                printf("玩家 %d 發言: %s\n", i + 1, recvline);
                bzero(&sendline, MAXLINE);
                sprintf(sendline, "玩家 %d 發言: %s\n", i + 1, recvline);
                broadcastMSG(room, sendline);
            }else{
                printf("玩家 %d 未發言，或連線中斷\n", i + 1);
                bzero(&sendline, MAXLINE);
                sprintf(sendline, "玩家 %d 未發言", i + 1);
                broadcastMSG(room, sendline);
            }
        }

        bzero(&sendline, MAXLINE);
        sprintf(sendline, "討論時間結束");
        broadcastMSG(room, sendline);
        
        int votes[room->capacity];
        int votes_num = 0;
        int alive_num = 0;
        for(int i = 0; i < room->capacity; i++){
            if(alive[i])
        ...
        printf("投票時間開始\n");
        broadcastMSG(room, "投票時間開始，請輸入玩家編號\n");

        while(votes_num < alive_num){
            fd_set read_fds;
            FD_ZERO(&read_fds);

            for(int i = 0; i < room->capacity; i++){
                if(alive[i] && room->onlineStatus[i]){
                    FD_SET(room->Connfds[i], &read_fds);
                }
            }

            int max_fd = getMaxFd(room->Connfds, room->capacity);
            int ready = select(max_fd + 1, &read_fds, NULL, NULL, NULL);

            if(ready > 0){
                for(int i = 0; i < room->capacity; i++){
                    if(alive[i] && room->onlineStatus[i] && FD_ISSET(room->Connfds[i], &read_fds)){
                        char recvline[MAXLINE];
                        bzero(recvline, MAXLINE);
                        int n = read(room->Connfds[i], recvline, MAXLINE);

                        if(n > 0){
                            int vote = atoi(recvline) - 1;
                            if(vote >= 0 && vote < room->capacity && alive[vote]){
                                votes[vote]++;
                                printf("玩家 %d 投給了玩家 %d\n", i + 1, vote + 1);
                            }else{
                                printf("玩家 %d 的投票無效。\n", i + 1);
        ...
        if(votedPlayer != -1){
            printf("玩家 %d 被淘汰。\n", votedPlayer + 1);
            alive[votedPlayer] = false;

            bzero(&sendline, MAXLINE);
            sprintf(sendline, "Out\n");
            writen(room->Connfds[votedPlayer], sendline, strlen((sendline)));

            bzero(sendline, MAXLINE);
            sprintf(sendline, "玩家 %d 被投票淘汰。\n", votedPlayer + 1);
            broadcastMSG(room, sendline);
        }else{
            printf("平票，沒有玩家被淘汰。\n");
            broadcastMSG(room, "平票，沒有玩家被淘汰。\n");
        }

        bzero(sendline, MAXLINE);
        sprintf(sendline, "投票時間結束\n");
        broadcastMSG(room, sendline);
